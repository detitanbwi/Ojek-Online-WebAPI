<x-app-layout>
    ...
    <!-- Session Messages -->
    @if(session('success'))
        <div class="p-4 mb-6 text-sm text-emerald-300 rounded-2xl bg-emerald-950/50 border border-emerald-900 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden transition-all duration-300 hover:shadow-2xl">
                <div class="p-6 sm:p-8 bg-gradient-to-br from-slate-900 to-indigo-950 text-white relative">
                ...
                <form id="createOrderForm" class="p-6 sm:p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div class="lg:col-span-5 space-y-6">
                            <div>
                                <label for="origin" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Titik Jemput (Origin)</label>
                                <div class="relative rounded-2xl shadow-sm flex items-center">
                                    <input type="text" id="origin" name="origin" required
                                        class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 pl-4 pr-32 text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200 text-sm"
                                        placeholder="Cari lokasi atau klik Pilih di Peta...">
                                    <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                        <button type="button" id="pickOriginOnMapBtn" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-xl text-indigo-400 bg-indigo-950 hover:bg-indigo-900 transition-colors">
                            ...
                            <div>
                                <label for="destination" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Titik Tujuan (Destination)</label>
                                <div class="relative rounded-2xl shadow-sm flex items-center">
                                    <input type="text" id="destination" name="destination" required
                                        class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 pl-4 pr-32 text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200 text-sm"
                                        placeholder="Cari lokasi atau klik Pilih di Peta...">
                                    <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                        <button type="button" id="pickDestOnMapBtn" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-xl text-indigo-400 bg-indigo-950 hover:bg-indigo-900 transition-colors">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                            Pilih
                                        </button>
                                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label for="price" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Tarif (Rupiah)</label>
                                    <div class="relative rounded-2xl shadow-sm">
                                        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                                            <span class="text-slate-500 text-sm">Rp</span>
                                        </div>
                                        <input type="number" id="price" name="price" required min="0"
                                            class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 pl-10 pr-4 text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200 text-sm font-bold text-indigo-400"
                                            placeholder="Tarif otomatis">
                                    </div>
                                </div>
                                
                                <div>
                                    <label for="driver_id" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Pilih Driver</label>
                                    <select id="driver_id" name="driver_id" required
                                        class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 px-4 text-slate-100 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200 text-sm">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label for="passenger_name" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Nama Penumpang (Opsional)</label>
                                    <input type="text" id="passenger_name" name="passenger_name"
                                        class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 px-4 text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200 text-sm"
                                        placeholder="Nama penumpang acak">
                                </div>
                                <div>
                                    <label for="payment_type" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Tipe Pembayaran</label>
                                    <select id="payment_type" name="payment_type" required
                                        class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 px-4 text-slate-100 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200 text-sm font-semibold">
                            ...
                            <!-- Fare Breakdown -->
                            <div id="fareBreakdown" class="hidden rounded-2xl p-4 text-xs bg-indigo-950/60 text-indigo-300 border border-indigo-900/50 space-y-1">
                        ...
                        <div class="lg:col-span-7 flex flex-col h-full min-h-[450px] border border-slate-800 rounded-3xl overflow-hidden relative shadow-inner bg-slate-950">
                            ...
                            <div class="absolute top-3 left-3 right-3 bg-slate-900/95 backdrop-blur-md px-4 py-2.5 rounded-2xl shadow-md border border-slate-800 flex items-center justify-between text-xs text-slate-200 z-10">
                                ...
                                <button type="button" id="resetMarkersBtn" class="text-rose-400 font-bold hover:underline hidden">Reset Rute</button>
                            </div>
        ...
        <div class="space-y-8">
            ...
            <div class="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden transition-all duration-300 hover:shadow-2xl">
                ...
                <div id="driverListContainer" class="divide-y divide-slate-800 max-h-[300px] overflow-y-auto">
                    @forelse($drivers->where('status_online', true) as $driver)
                        <div class="p-5 flex items-center justify-between hover:bg-slate-800/20 transition-all duration-150">
                            <div class="space-y-1">
                            ...
                            <div class="flex items-center gap-3">
                                <button onclick="detachDriver({{ $driver->id }})" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-bold rounded-xl text-rose-400 bg-rose-950/50 hover:bg-rose-900/50 transition-colors">
                    ...
                    @empty
                        <div class="p-8 text-center text-slate-500 text-sm">Tidak ada driver online saat ini.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden transition-all duration-300 hover:shadow-2xl">
                ...
                <form action="{{ route('admin.settings.save') }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    <div>
                        <label for="commission_type" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Tipe Komisi</label>
                        <select name="commission_type" id="commission_type" onchange="toggleRoundingOption()"
                            class="w-full rounded-2xl border-slate-800 bg-slate-950 py-3 px-4 text-sm text-slate-100 focus:border-indigo-500 focus:bg-slate-900 focus:ring-indigo-500 transition-all duration-200">
                    ...
                    <div>
                        <label for="commission_value" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Nilai Potongan</label>
                    ...
                    <div id="roundOptionContainer" class="{{ ($settings['commission_type'] ?? 'percentage') === 'fixed' ? 'hidden' : '' }}">
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="round_hundreds_down" value="true" class="rounded border-slate-700 text-indigo-600 bg-slate-950 focus:ring-indigo-500" {{ ($settings['round_hundreds_down'] ?? 'true') === 'true' ? 'checked' : '' }}>
                            <span class="text-xs font-semibold text-slate-400">Bulatkan komisi ke ratusan kebawah</span>
                        </label>
                    </div>
    ...
    <div class="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden transition-all duration-300 hover:shadow-2xl mt-8">
        <div class="p-6 bg-gradient-to-br from-slate-900 to-indigo-950 text-white relative overflow-hidden flex items-center justify-between">
            <div class="absolute right-0 top-0 opacity-10 pointer-events-none transform translate-x-4 -translate-y-4 scale-150 text-white">
                <svg width="120" height="120" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-950/50 border-b border-slate-800">
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">ID</th>
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Driver</th>
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Rute (Jemput → Tujuan)</th>
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Tarif Cust</th>
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Net Driver</th>
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Pot Admin</th>
                        ...
                        <th class="p-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody id="ordersTableBody" class="divide-y divide-slate-800 text-sm text-slate-300">
                    @forelse($orders as $order)
                        <tr class="hover:bg-slate-800/30 transition-colors">
                            <td class="p-4 font-mono font-semibold text-indigo-400">#{{ $order->id }}</td>
                            <td class="p-4 font-medium">{{ $order->driver->name ?? 'N/A' }}</td>
                            <td class="p-4">
                                <div class="font-medium text-slate-200 truncate max-w-xs" title="{{ $order->origin }}">{{ $order->origin }}</div>
                                <div class="text-xs text-slate-500 mt-0.5 truncate max-w-xs" title="{{ $order->destination }}">→ {{ $order->destination }}</div>
                            </td>
                            <td class="p-4 font-semibold text-slate-200">Rp {{ number_format($order->price, 0, ',', '.') }}</td>
                            <td class="p-4 font-semibold text-emerald-400">Rp {{ number_format($order->driver_fare, 0, ',', '.') }}</td>
                            <td class="p-4 font-semibold text-rose-400">Rp {{ number_format($order->admin_fee, 0, ',', '.') }}</td>
                            <td class="p-4">
                                @if($order->status === 'pending')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20">Pending</span>
                                @elseif($order->status === 'accepted')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">Accepted</span>
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/20">{{ ucfirst($order->status) }}</span>
                                @endif
                            </td>
                            <td class="p-4 text-xs text-slate-500">{{ $order->created_at->diffForHumans() }}</td>
                            <td class="p-4 text-center">
                                <button onclick="viewOrderDetail({{ $order->id }})" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-xl text-indigo-400 bg-indigo-950 hover:bg-indigo-900 transition-colors">Detail</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="p-8 text-center text-slate-500 text-sm">Belum ada orderan yang dibuat.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="orderDetailModal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-slate-950/80 backdrop-blur-sm flex justify-center items-center p-4">
        <div class="bg-slate-900 rounded-3xl w-full overflow-hidden shadow-2xl border border-slate-805 transform transition-all duration-300" style="max-width: 512px;">
            <div class="p-6 text-white flex justify-between items-center bg-gradient-to-r from-slate-900 to-indigo-950 border-b border-slate-800">
            ...
            <div class="p-6 space-y-6">
                <div class="space-y-3">
                    <div>
                        ...
                        <span id="detail_origin" class="text-sm font-semibold text-slate-200"></span>
                    </div>
                    <div>
                        <span class="block text-[10px] uppercase font-bold text-slate-500">Titik Tujuan (Destination)</span>
                        <span id="detail_destination" class="text-sm font-semibold text-slate-200"></span>
                    </div>
                </div>
                <div class="border-t border-slate-800 pt-4 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <span id="detail_driver_name" class="text-sm font-semibold text-slate-200"></span>
                    </div>
                    <div>
                ...
                <div class="border-t border-slate-800 pt-4 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <span id="detail_passenger_name" class="text-sm font-semibold text-slate-200"></span>
                    </div>
                    <div>
                        <span class="block text-[10px] uppercase font-bold text-slate-500">Metode Bayar</span>
                        <span id="detail_payment_type" class="text-sm font-semibold text-slate-200"></span>
                    </div>
                </div>
                <div class="bg-slate-950/60 rounded-2xl p-4 space-y-2 border border-slate-800">
                    <span class="block text-[10px] uppercase font-bold text-slate-550 mb-1">Rincian Komisi</span>
                    <div class="flex justify-between text-xs">
                        <span class="text-slate-400">Tarif Penumpang (Customer Pays):</span>
                        <span id="detail_price" class="font-bold text-slate-200"></span>
                    </div>
                    ...
                    <div class="flex justify-between text-sm font-bold text-emerald-450 border-t border-slate-850 pt-2">
                        <span>Pendapatan Bersih Driver:</span>
                        <span id="detail_driver_fare"></span>
                    </div>
                </div>
            </div>
            <div class="p-6 bg-slate-950/30 border-t border-slate-800 flex justify-end gap-3">
    ...
    <div id="detachModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-3xl bg-slate-900 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-800">
                <div class="bg-slate-900 px-6 pb-6 pt-8 sm:p-8 sm:pb-6">
                    <div class="sm:flex sm:items-start">
                        <!-- Icon Warning -->
                        <div class="mx-auto flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-rose-950 text-rose-450 sm:mx-0 sm:h-12 sm:w-12 border border-rose-900/50">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                        </div>
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-lg font-bold leading-6 text-slate-100" id="modal-title">Konfirmasi Detach</h3>
                            <div class="mt-3">
                                <p class="text-sm text-slate-400">Apakah Anda yakin ingin memutuskan (detach) pengemudi ini? Driver akan otomatis terputus dari sistem admin dan dialihkan ke status offline secara realtime.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="bg-slate-950/50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-850">
                    ...
                    <button type="button" onclick="closeDetachModal()" class="inline-flex w-full justify-center rounded-xl bg-slate-800 px-4 py-2.5 text-sm font-bold text-slate-300 hover:bg-slate-700 sm:w-auto transition-colors">Batal</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
            ...
            map = new google.maps.Map(document.getElementById('map'), {
                ...
                styles: [
                    ...
                    },
                    {
                        ...
                        "stylers": [{"color": "#141921"}, {"lightness": 14}, {"weight": 1.4}]
                    },
                    {
                        "featureType": "landscape",
                        "elementType": "all",
                        "stylers": [{"color": "#080e15"}]
                    },
                    {
                        "featureType": "poi",
                        "elementType": "geometry",
                        "stylers": [{"color": "#0c131f"}, {"lightness": 5}]
                    },
                    {
                    ...
                    },
                    {
                        "featureType": "road.highway",
                        "elementType": "geometry.stroke",
                        "stylers": [{"color": "#25334d"}, {"lightness": 25}]
                    },
                    {
        ...
        function showFareBreakdown(distanceKm, ceilKm, price) {
            ...
            if (ceilKm <= 3) {
                fareBreakdown.innerHTML = `
                    ...
                    <div class="font-bold flex justify-between text-indigo-400"><span>Jarak Bulat:</span> <span>${ceilKm} Km</span></div>
                    <hr class="my-1 border-indigo-900">
                    <div class="flex justify-between font-bold text-indigo-300 text-sm mt-0.5"><span>Tarif Flat (≤ 3 Km):</span> <span>Rp 8.000</span></div>
                `;
            } else {
                ...
                fareBreakdown.innerHTML = `
                    ...
                    <hr class="my-1 border-indigo-900">
                    <div class="flex justify-between font-bold text-indigo-300 text-sm mt-0.5"><span>Total Tarif:</span> <span>${formatRupiah(price)}</span></div>
                `;
            }
        }
        ...
        document.getElementById('createOrderForm').addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    responseMsg.className = 'block rounded-2xl p-4 text-sm bg-emerald-950/50 text-emerald-300 border border-emerald-900 space-y-1';
                    ...
                    responseMsg.className = 'block rounded-2xl p-4 text-sm bg-rose-955/50 text-rose-350 border border-rose-900';
                    let errorsHtml = '';
                    if (result.errors) {
        ...
        async function viewOrderDetail(orderId) {
            try {
                ...
                if (result.success) {
                    ...
                    let badgeHtml = '';
                    if (order.status === 'pending') {
                        badgeHtml = '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20">Pending</span>';
                    } else if (order.status === 'accepted') {
                        badgeHtml = '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">Accepted</span>';
                    } else if (order.status === 'completed') {
                        badgeHtml = '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Completed</span>';
                    } else {
                        badgeHtml = `<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-500/10 text-rose-450 border border-rose-500/20">${order.status}</span>`;
                    }
        ...
        function triggerRepeatOrder() {
            ...
            if (orderFormElement) {
                ...
                const formCard = orderFormElement.closest('.bg-slate-900');
                if (formCard) {
        ...
        function updateBreakdownPreview() {
            ...
            if (priceVal > 0) {
                ...
                breakdownEl.innerHTML = `
                    <div class="flex justify-between"><span>Tarif Penumpang:</span><span class="font-bold text-slate-100">${formatRupiah(priceVal)}</span></div>
                    <div class="flex justify-between text-rose-400"><span>Potongan Admin (${type === 'percentage' ? val + '%' : 'Nominal'}):</span><span>- ${formatRupiah(adminFee)}</span></div>
                    <div class="flex justify-between text-emerald-400 font-bold"><span>Pendapatan Driver:</span><span>${formatRupiah(driverFare)}</span></div>
                `;
            } else {
